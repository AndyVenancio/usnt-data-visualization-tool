...
interface PlayerRowProps {
  ...
  comparing: boolean;
  selectingPlayer: (player: Player) => void;
  deselectingPlayer: (player: Player) => void;
  selected: boolean;
}
...
const PlayerRow: React.FC<PlayerRowProps> = ({
  ...
  return (
    ...
    // Row must contain checkbox if comparing
    // Player Row must react to hover
    // Clicking on the row links to the player page

    <div
      className="flex flex-col items-center justify-center w-full p-4 border-b border-gray-200 hover:bg-gray-100"
      onClick={(comparing ? () => { if (selected) { deselectingPlayer(player); } else { selectingPlayer(player); } } : () => onClickPlayer(player))}
      ...
      <div className="flex flex-row items-center w-full">
        ...
        <div className="flex flex-col w-2/5">
          ...
          <span className="text-sm text-gray-500">
            {age_in_years} years | {competition_name} | {position_general}
          </span>
        </div>
        ...
        <div className="flex flex-row w-3/5 justify-evenly">
          <div className="flex flex-col items-center justify-center px-4">
            <div className="flex flex-row items-baseline space-x-1">
              ...
              <h1 className="text-sm text-bold text-gray-600">
                {topStats[0][3]}
              </h1>
              <p className="text-xs text-gray-600">(per 90)</p>
            </div>
            <span className="text-sm text-gray-600">
              {translateStatName(topStats[0][2])}
            </span>
          </div>
          ...
          <div className="flex flex-col items-center justify-center px-4">
            ...
            <span className="text-sm text-gray-600">
              {translateStatName(topStats[1][2])}
            </span>
          </div>
        </div>
