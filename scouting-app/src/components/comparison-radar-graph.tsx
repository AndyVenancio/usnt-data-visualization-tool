...
const PlayerRadarChart: React.FC<PlayerRadarChartProps> = ({ players }) => {
  ...
  useEffect(() => {
    ...
    const data1 = cleanDataRadar(players[0]);
    const data2 = cleanDataRadar(players[1]);
    ...
    // Draw axes and labels for each statistic
    data1.forEach((d, i) => {
    ...
    // Draw radar areas for each player
    const drawRadarArea = (
    ...
    drawRadarArea(data1, "#10B981");
    drawRadarArea(data2, "#6366F1");

    // Add percentiles as text labels
    const placePercentiles = (data: { axis: string; value: number }[]) => {
      data.forEach((d, i) => {
        const angle = angleSlice * i - Math.PI / 2;
    ...
    placePercentiles(data1);
    placePercentiles(data2);
